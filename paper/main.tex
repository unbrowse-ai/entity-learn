...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners=3pt, minimum width=1.8cm, minimum height=0.7cm, font=\small, fill=codebg},
  store/.style={draw, rounded corners=3pt, minimum width=2.0cm, minimum height=0.7cm, font=\small, fill=accentblue!10},
...
% External
\node[right=0.9cm of cache, font=\small\itshape, text=codegray, align=center] (apis) {GitHub, Gmail,\\Calendar, CRM, \ldots};
...
\end{tikzpicture}
\caption{Entity-Learn architecture. The pointer store holds command strings and inferred schemas. At query time, Resolve re-executes commands (with TTL caching), ranks results with BM25, and links entities through relation discovery. Bootstrap auto-populates pointers from session history.}
...
\IF{$J$ is array}
  \STATE shape $\leftarrow$ \texttt{array}; $S \leftarrow J[0{:}5]$
\ELSIF{$J$ is object-of-objects}
  \STATE shape $\leftarrow$ \texttt{ooo}; $S \leftarrow \text{vals}(J)[0{:}5]$
\ELSIF{$J$ is object}
  \STATE shape $\leftarrow$ \texttt{object}; $S \leftarrow [J]$
\ELSE
  \RETURN $\bot$
\ENDIF
\STATE $K \leftarrow \bigcup_{s \in S} \text{keys}(s)$
\STATE $\text{id} \leftarrow \text{\textsc{Match}}(K, \text{ID\_C})$
\STATE $\text{title} \leftarrow \text{\textsc{Match}}(K, \text{TTL\_C})$
\STATE $\text{stat} \leftarrow \text{\textsc{Match}}(K, \text{STAT\_C})$
\STATE $\text{type} \leftarrow \text{\textsc{InferType}}(c)$
\STATE $\gamma \leftarrow 0.5 {+} 0.2[\text{id}] {+} 0.2[\text{title}] {+} 0.1[\text{stat}]$
\RETURN $p(c, \text{type}, \text{shape}, K, \gamma)$
\end{algorithmic}
\end{algorithm}
...
\begin{equation}
\text{score}(e, q) = \sum_{t \in q} \text{IDF}(t) \cdot \frac{f(t, e) (k_1 {+} 1)}{f(t, e) + k_1 (1 {-} b {+} b \frac{|e|}{\overline{|e|}})}
\end{equation}
...
\begin{table}[t]
\centering\small
\caption{Cross-source retrieval results (production, 50 pointers, 31 types)}
\label{tab:retrieval}
\begin{tabular}{@{}lrr@{}}
\toprule
\textbf{Query mode} & \textbf{Entities} & \textbf{Latency} \\
\midrule
Full resolve (cold, 50 ptrs) & 400 & 17.7s \\
Search (cold, 11 types) & 75 & 25.5s \\
Type-filtered (cold) & 20 & 1.3s \\
Type-filtered (warm) & 20 & 57ms \\
Search + relations (warm) & 75 & 128ms \\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
\textbf{Test} & \textbf{Snapshot} & \textbf{Live} & \textbf{OK} \\
\midrule
GH issue create & 20 (frozen) & 21 (live) & Yes \\
CRM mutation & orig.\ status & mutated & Yes \\
npm match & N/A & 3{,}125 = 3{,}125 & Yes \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}lrrl@{}}
\toprule
\textbf{Category} & \textbf{BM25} & \textbf{Vec.} & \textbf{Win} \\
\midrule
Identifier (\texttt{\#76}) & 0\% & 100\% & Vec. \\
Keyword & 100\% & 100\% & Tie \\
Version tag & 100\% & 100\% & Tie \\
Entity name & 100\% & 100\% & Tie \\
Status-based & 30\% & 30\% & Tie \\
\midrule
\textbf{Overall mean} & \textbf{73.6\%} & \textbf{95.0\%} & \textbf{Vec.} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Type} & \textbf{Correct} & \textbf{Total} & \textbf{Prec.} \\
\midrule
