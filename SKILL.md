...
- When you need to remember where data lives across CLIs, APIs, and files
- When you want cross-source search (issues + deals + emails + metrics in one query)
- When you're building a generative UI that needs to render live data
- When an agent should learn from its own tool calls
- Instead of copying data into a store that goes stale
...
entity-learn learn "cat data.json" --type custom_thing --ttl 60
...
If `--search` is provided, all results are ranked using BM25 (TF-IDF with document length normalization). Search works across entity types — "Maven fundraising" finds deals, "windows crash" finds issues.
...
That's it. No vector DB. No knowledge graph. No embeddings. Just a list of shell commands and the schema of what they return.
...
Before searching manually, check if a pointer exists:
  npx tsx ~/.agent-org/lib/entity-learn.ts query --search "your question"

When you discover a useful command that returns JSON, teach it:
  npx tsx ~/.agent-org/lib/entity-learn.ts learn "the command" --type entity_type

The system learns from your tool calls and builds a searchable index of pointers.
```

## Integration with generative UI

The resolver output is a normalized JSON entity. Point `json-render` at it for instant list+detail views of any entity type. New types auto-appear as you teach new sources.
...
- `lib/entity-learn.ts` — core: learn, resolve, BM25, CLI (single file, zero deps)
- `pointers.json` — the "database" (just command recipes + field mappings)
